...
<br> [Aaron Salot's Assignment 3 Github](https://github.com/aaronsalot/Assignment_3)
...
```{r}
best_parameter <- lasso_tune %>% 
  select_best(metric = "accuracy")

lasso_final_wf <- lasso_wf %>% 
  finalize_workflow(best_parameter)

lending_lasso_final_mod <- lasso_final_wf %>% 
...
```{r}
lending_lasso_test <- lasso_final_wf %>% 
...
rfpenalty_grid <- grid_regular(finalize(mtry(), lending_training %>%
...
```{r}
best_parameter_rf <- rf_tune %>%
  select_best(metric = "accuracy")

rf_wf_final <- rf_workflow %>% 
  finalize_workflow(best_parameter_rf)

lending_final_mod <- rf_wf_final %>%
  fit(lending_training)

rf_last_fit <- rf_wf_final %>% 
  last_fit(lending_split)
```
